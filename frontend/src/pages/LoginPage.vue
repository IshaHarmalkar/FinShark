          ...
          <q-form class="q-gutter-md" @submit.prevent="submitForm">
            ...
            <div>
              ...
              <div class="text-center q-mt-sm q-gutter-lg">
                ...
                <router-link class="text-white" to="/login">Create Account</router-link>
              </div>
            </div>
          </q-form>
        </q-card-section>
  ...
  methods: {
    async submitForm() {
      ...
      if (!this.login.username || !this.login.password) {
        ...
        try {
          await auth.doLogin(this.login)
          const toPath = this.$route.query.to || 'admin'
          this.$router.push(toPath)
        } catch (err) {
          if (err.response.data.detail) {
            $q.notify({ type: 'negative', message: err.response?.data?.detail || 'Login failed' })
          }
        }
      }
    },
